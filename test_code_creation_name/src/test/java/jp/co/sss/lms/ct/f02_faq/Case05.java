...
public class Case05 {
	...
	void test05() throws InterruptedException {
		// TODO ここに追加
		final WebElement faqSearchInput = webDriver.findElement(By.className("form-control"));
		final WebElement searchButton = webDriver.findElement(By.cssSelector("input[value='検索']"));

		String searchWord = "申し込み";

		faqSearchInput.clear();
		faqSearchInput.sendKeys(searchWord);

		searchButton.click();
		
		getEvidence(new Object() {});

		//2秒後に画面下部へ2000px分(最下部)移動
		Thread.sleep(2000);

		scrollTo("1000");
		
		getEvidence(new Object() {},"result");

		//上記処理実行後、2秒後に画面最上部へ遷移
		Thread.sleep(2000);

		scrollTo("0");
		
		Boolean applicationCheck = webDriver.getPageSource().contains("申し込み");
	...
	void test06() throws InterruptedException {
		...
		final WebElement inputForm = webDriver.findElement(By.className("form-control")); 
		
		Thread.sleep(1000);

		getEvidence(new Object() {},"beforeErase");

		clearButton.click();
		
		getEvidence(new Object() {},"deleted");
		
		
		assertEquals(inputForm.getText(), "");
	}
